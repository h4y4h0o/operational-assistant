...
| 2 | HTTP Request (flights) | `httpRequest` GET | Charge flights.json |
| 3 | HTTP Request (incidents) | `httpRequest` GET | Charge incidents.json |
...
| 8 | Build Slack Message | `code` | Construit le message dynamique |
| 9 | Slack Alerting | `httpRequest` POST | Envoie via Incoming Webhook |
...
Le node n8n "Read/Write from Disk" retourne du binaire non parsé.
La solution retenue : serveur Python `http.server` exposant les fichiers,
consommés par un node HTTP Request qui parse automatiquement le JSON.
